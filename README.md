...
- 插件卡片提供两个复制动作：复制可被 Codex Desktop clone 的插件根仓库 URL、复制 `codex plugin marketplace add <插件仓库链接> --ref <默认分支>` CLI 安装命令。
...
服务端检测插件时会读取 Git 远端 HEAD 对应的默认分支，例如 `main`、`master` 或 `dev`。网页中的“复制仓库链接”会复制 `https://github.com/owner/repo` 这种 Git 根仓库 URL，确保 Codex Desktop 可以直接 clone；默认分支仍会以标签显示，并写入 `repositoryTreeUrl` 供溯源查看。“复制 CLI 命令”会复制 `codex plugin marketplace add https://github.com/owner/repo --ref <默认分支>`，避免非 `main` 仓库安装到错误分支。
...
通过时返回 `status: "approved"` 和插件列表；插件会包含 `defaultBranch`、`headSha`、`repositoryTreeUrl` 等默认分支元信息。失败时返回 `status: "failed"` 与失败原因，并在市场状态中保留失败记录。
