...
- 插件卡片提供两个复制动作：复制插件默认分支浏览链接、复制 `codex plugin marketplace add <插件仓库链接> --ref <默认分支>` CLI 安装命令。
...
服务端检测插件时会读取 Git 远端 HEAD 对应的默认分支，例如 `main`、`master` 或 `dev`。网页中的“复制仓库链接”会复制 `https://github.com/owner/repo/tree/<默认分支>`，方便 Desktop 用户确认安装来源；“复制 CLI 命令”会复制 `codex plugin marketplace add https://github.com/owner/repo --ref <默认分支>`，避免非 `main` 仓库安装到错误分支。
